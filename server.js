  ...
  try {
    ...
    for (const day of reminderDays) {
      ...
      for (const user of usersToRemind) {
        const subject = `Importante: Novo App do Atitude Kids - Atualização de CAC`;
        const dueDate = dayjs(user.issued_at).add(6, 'months').format('DD/MM/YYYY');

        const htmlBody = `
                  <p>Olá, ${user.nome}!</p>
                  <p>Estamos migrando para um novo aplicativo para melhor gerenciar nosso ministério.</p>
                  <p>A data de emissão do seu último CAC completará 6 meses em <strong>${dueDate}</strong>. Para renovar, por favor, acesse o <strong>novo app EngageSuite</strong> pelo link abaixo e envie seu documento atualizado por lá:</p>
                  <p><a href="https://www.engagesuite.com.br/o/igreja-batista-atitude-da-barra/home" style="display:inline-block; padding:10px 20px; background:#4f46e5; color:#fff; text-decoration:none; border-radius:5px;">Acessar Novo App</a></p>
                  <p><strong>Veja como fazer:</strong></p>
                  <p><img src="${process.env.APP_BASE_URL || ''}/public/tutorial_novo_app_1.jpg" alt="Tutorial Passo 1" style="max-width:100%; border:1px solid #ddd; margin-bottom:10px;"></p>
                  <p><img src="${process.env.APP_BASE_URL || ''}/public/tutorial_novo_app_2.jpg" alt="Tutorial Passo 2" style="max-width:100%; border:1px solid #ddd;"></p>
                  <p>Agradecemos seu serviço e dedicação!</p>
        ...
        await sendPushNotification(user.id, {
          title: `Novo App Atitude Kids`,
          body: `Estamos de casa nova! Sua renovação do CAC deve ser feita no novo app. Verifique seu e-mail.`,
          url: `https://www.engagesuite.com.br/o/igreja-batista-atitude-da-barra/home`
        });
        pushSent++;
      }
    }
    ...
    for (const expiredUser of expiredUsersNeedingReminder) {
      ...
      const weeklyHtml = `
              ...
              <p>Identificamos que seu CAC venceu em <strong>${expiredDate}</strong>.</p>
              <p>Estamos com um <strong>novo aplicativo</strong>! Para regularizar seu cadastro, por favor, envie o novo documento através do link abaixo (EngageSuite):</p>
              <p><a href="https://www.engagesuite.com.br/o/igreja-batista-atitude-da-barra/home" style="display:inline-block; padding:10px 20px; background:#4f46e5; color:#fff; text-decoration:none; border-radius:5px;">Acessar Novo App</a></p>
              <p>O processo é simples:</p>
              <p><img src="${process.env.APP_BASE_URL || ''}/public/tutorial_novo_app_1.jpg" alt="Tutorial Passo 1" style="max-width:100%; border:1px solid #ddd; margin-bottom:10px;"></p>
              <p><img src="${process.env.APP_BASE_URL || ''}/public/tutorial_novo_app_2.jpg" alt="Tutorial Passo 2" style="max-width:100%; border:1px solid #ddd;"></p>
              <p>Contamos com você!</p>
              <p>Liderança Atitude Kids</p>
            `;
      ...
      await sendPushNotification(expiredUser.id, {
        title: `Novo App - Envio de CAC`,
        body: `Seu CAC venceu. Acesse o novo app EngageSuite para enviar o documento atualizado.`,
        url: `https://www.engagesuite.com.br/o/igreja-batista-atitude-da-barra/home`
      });
      pushSent++;
    }
